...
const DonateItem = () => {
  ...
  // Get current user if authenticated and autofill donor info
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      setCurrentUser(user);
      if (user) {
        // Autofill donor information from logged-in user
        setDonorId(user.uid);
  ...
  const handleDonate = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, "donations"), {
        ...
        donorId: donorId || (currentUser ? currentUser.uid : null), // userID
        donorName: donorName || (currentUser ? (currentUser.displayName || currentUser.email || "Anonymous") : ""), // userName
        donorEmail: donorEmail || (currentUser ? currentUser.email : null), // userEmail
    ...
    <div className="min-h-screen bg-[#9af71e]/5 bg-[linear-gradient(to_right,#7db03815_1px,transparent_1px),linear-gradient(to_bottom,#7db03815_1px,transparent_1px)] [background-size:24px_24px] pb-20">
      ...
      <div className="bg-white/90 backdrop-blur-md px-4 py-4 shadow-sm sticky top-0 z-10 border-b border-[#7db038]/20">
        <div className="max-w-3xl mx-auto flex items-center gap-4">
          <Link to="/donationcorner" className="text-gray-600 hover:text-[#7db038] transition-colors"><ArrowLeft size={24} /></Link>
          <h1 className="text-xl font-bold text-[#364f15]">Donate an Item</h1>
        </div>
      </div>

      <main className="max-w-3xl mx-auto px-4 mt-6">
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-[#7db038]/20 space-y-6">
            ...
            <div>
                ...
                <div className="flex gap-3 overflow-x-auto pb-2">
                    ...
                    {images.length < 3 && (
                        <label className="w-24 h-24 flex flex-col items-center justify-center border-2 border-dashed border-[#7db038]/40 rounded-xl cursor-pointer bg-[#7db038]/5 hover:bg-[#7db038]/10 text-[#7db038]">
                            {isAnalyzingImage ? (
                                <Loader2 className="animate-spin text-[#7db038]" size={20} />
                            ) : (
                                <>
                                    <Upload size={20} />
            ...
            <div>
                <h2 className="text-sm font-bold text-[#364f15] uppercase tracking-wider mb-4">Your Details</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-1">Your Name</label>
                        <input type="text" value={donorName} onChange={e => setDonorName(e.target.value)} placeholder="Display Name" className="w-full p-3 rounded-xl border border-gray-200 focus:border-[#7db038] focus:ring-2 focus:ring-[#7db038]/20 outline-none" />
                    </div>
                    <div>
                        <label className="block text-xs font-semibold text-gray-500 mb-1">Email / Contact (Optional)</label>
                        <input type="text" value={donorEmail} onChange={e => setDonorEmail(e.target.value)} placeholder="How can they reach you?" className="w-full p-3 rounded-xl border border-gray-200 focus:border-[#7db038] focus:ring-2 focus:ring-[#7db038]/20 outline-none" />
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-4">
                <h2 className="text-sm font-bold text-[#364f15] uppercase tracking-wider mb-4">Item Details</h2>
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select value={condition} onChange={e => setCondition(e.target.value)} className="w-full p-3 rounded-xl border border-gray-200 focus:border-[#7db038] focus:ring-2 focus:ring-[#7db038]/20 outline-none bg-white">
                ...
                <div>
                    ...
                    <textarea value={keywords} onChange={e => setKeywords(e.target.value)} placeholder="e.g. Science textbook, used for 2 semesters, good condition" className="w-full p-3 rounded-xl border border-gray-200 focus:border-[#7db038] focus:ring-2 focus:ring-[#7db038]/20 outline-none h-24 resize-none" />
                ...
                <div>
                    ...
                    <input type="text" value={title} onChange={e => setTitle(e.target.value)} className="w-full p-3 rounded-xl border border-gray-200 focus:border-[#7db038] focus:ring-2 focus:ring-[#7db038]/20 outline-none font-bold text-gray-800" placeholder="e.g. Science Textbook" />
                </div>
                ...
                <div>
                    ...
                    {locationSelection === "Other" && (
                        <input type="text" placeholder="Enter specific location..." value={customLocation} onChange={e => setCustomLocation(e.target.value)} className="w-full p-3 rounded-xl border border-[#7db038]/40 bg-[#7db038]/5 focus:border-[#7db038] focus:ring-2 focus:ring-[#7db038]/20 outline-none" />
                    )}
                </div>

                <div>
                    <label className="block text-xs font-semibold text-gray-500 mb-1">Description (Optional)</label>
                    <textarea value={description} onChange={e => setDescription(e.target.value)} className="w-full p-3 rounded-xl border border-gray-200 focus:border-[#7db038] focus:ring-2 focus:ring-[#7db038]/20 outline-none h-24" placeholder="Briefly describe the item..." />
                </div>
            </div>

            {/* Submit Button */}
            <button onClick={handleDonate} disabled={isSubmitting} className="w-full bg-[#7db038] text-white font-bold py-4 rounded-2xl shadow-lg hover:bg-[#4a6b1d] transition-all active:scale-95 text-lg flex justify-center items-center gap-2 disabled:opacity-50">
